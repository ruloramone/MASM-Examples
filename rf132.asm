...
long 	DB	"5"
longact	DB	?
ascii	DB 00,30H,30H,30H,30H,30H,30H,'$'
asciz	DB 00,0FFH,0FFH,0FFH,0FFH,0FFH,0FFH,'$'
ascix	DB 00,0FFH,0FFH,0FFH,0FFH,0FFH,0FFH,'$'
VAR1	DB 100
...
ajus:	cmp segs+1,32h
		...
		jne uno
		jmp zeta
rock:	MOV AH,02	;mover cursor
        MOV BH,00	;para ingresar datos
        MOV DH,05	; renglon
        MOV DL,34	;columna
        INT 10H	
		MOV AH, 0AH
...
;------------------------------------------------------------
Retardo Proc Near
SGN1:	MOV DX,0ffh     ;CONTADOR EXTERNODE xx VUELTAS
...
;------------------------------------------------------------
Fuego	Proc	Near
		mov BH,asciz+1		;detectando la alarma
		cmp bh,segs+1
		je wii
		jmp hell
wii:	mov bh,asciz+2
