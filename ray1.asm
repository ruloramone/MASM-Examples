...
ini: 
        ...
        mov cx, ivert              ;Posicion columna 64 
c20:    mov dx, ihort            ;Posicion renglon 70
...
;--------------------------------------vertical derecha-----------------------------------
		mov bx, 00
		mov dx, ihort		;Posicion renglon 70
c90:	mov cx, overt       ;Posicion columna 300
        mov ah, 0ch             ;Funcion dibujar pixel
        mov al, 03
        int 10h
       	inc dx						;Incrementa columna
        cmp dx,270       ;Prueba si llego al limite establecido
        jb c90
;---------------------------------------------horizontal inferior----------------------------------        
		mov bx, 00
		mov cx, ihort        ;Posicion columna
c30:    mov dx, ohort        ;Posicion renglon
        mov ah, 0ch             ;Funcion dibujar pixel
        mov al, 03
        int 10h
        dec cx                  ;Incrementa columna
        cmp cx,64              ;Prueba si llego al limite establecido
        jb c30
;--------------------------------------vertical izquierda----------------------------------------------

		mov bx, 00
		mov dx, ohort		;Posicion renglon
c77:	mov cx,	ivert        ;Posicion columna
        mov ah, 0ch             ;Funcion dibujar pixel
        mov al, 03
        int 10h
       	inc dx						;Incrementa columna
        cmp dx,70       ;Prueba si llego al limite establecido
        jb c77
;------------------------------- repetir cuadro------------------------------
		add ihort,5
		add ivert,5		
